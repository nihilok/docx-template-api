...
from .constants import SECRET_KEY, ALGORITHM, GUEST_IDS, SUPER_USERS, ACCESS_TOKEN_EXPIRE_MINUTES
...
async def authenticate_user(username: str, password: str) -> Optional[UserPydantic]:
    user = await UserPydantic.from_queryset_single(User.get(name=username))
    try:
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    try:
        ...
        raise credentials_exception
    user = await UserPydantic.from_queryset_single(User.get(name=token_data.username))
    if user is None:
        raise credentials_exception
    return user
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()) -> Token:
    ...
    print('done create access token')
...
async def check_superuser(user: UserPydantic =
                          Depends(get_current_user)) -> Optional[UserPydantic]:
...
async def create_user(user: UserPydanticIn,
                      premises_id: int,
                      superuser: UserPydantic = Depends(check_superuser)):
    if superuser:
        user_obj = User(name=user.name, password_hash=get_password_hash(user.password_hash), premises_id=premises_id)
        await user_obj.save()
...
async def create_premises(premises: PremisesPydanticIn,
                          superuser: UserPydantic = Depends(check_superuser)):
    if superuser:
        prem_obj = await Premises.create(name=premises.name)
        await prem_obj.save()
        return await PremisesPydantic.from_tortoise_orm(prem_obj)
    else:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="You need to be a superuser to create another user.",
